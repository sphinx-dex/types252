...
impl I128Impl of I128Trait {
    #[inline(always)]
    fn new(val: u128, sign: bool) -> i128 {
        i128 { val, sign: if val == 0 { false } else { sign } }
    }
...
impl Felt252TryIntoI128 of TryInto<felt252, i128> {
    ...
    fn try_into(self: felt252) -> Option<i128> {
        let option: Option<u128> = felt_abs(self).try_into();
        match option {
            Option::Some(val) => Option::Some(
                I128Trait::new(option.unwrap(), felt_sign(self))
            ),
            Option::None(()) => Option::<i128>::None(())
        }
    }
}
